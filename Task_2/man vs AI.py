...
def inputCorrectCount(name, c):
    while True:
        try:
            x = int(input(f'{name}, введите количество конфет, которое возьмете (от 1 до {c}): '))
        except ValueError:
            print(f'Ошибка - необходимо ввести число от 1 до {c}!')
            continue    
        if x < 1 or x > c:
            print(f'{name}, введите корректное количество конфет (от 1 до {c})!: ')
            continue
        break
    return x
...
while candies_count > max_step:
    if turn:
        ...
        x = getSmart(candies_count, max_step)
        score2 += x
        candies_count -= x
        turn = True
